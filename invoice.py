...
#            on_change=['list_price', 'rebate'],
    ...
    def __init__(self):
        ...
        self._rpc.setdefault('on_change_quantity', False)
        self._rpc.setdefault('on_change_unit', False)
        if self.unit_price.on_change:
            self.unit_price.on_change = copy.copy(self.unit_price.on_change)
        else:
            self.unit_price.on_change = []
        for fname in ('unit_price', 'list_price'):
            if fname not in self.unit_price.on_change:
                self.unit_price.on_change.append(fname)
        self._rpc.setdefault('on_change_unit_price', False)
        if 'rebate' not in self.amount.on_change_with:
            self.amount= copy.copy(self.amount)
            self.amount.on_change_with = copy.copy(self.amount.on_change_with)
            self.amount.on_change_with.append('rebate')
        self._reset_columns()
    ...
    def _compute_list_price(self, product, currency, date, type_):
        pool = Pool()
    ...
    def on_change_product(self):
        ...
        result = super(InvoiceLine, self).on_change_product(values)
        ...
        if values.get('product'):
            ...
            list_price = self._compute_list_price(product, currency,
        ...
        result['rebate'] = self._compute_rebate(list_price,
            result['unit_price'])
...
#            result = {}
    ...
    def on_change_unit_price(self):
        try:
            super(InvoiceLine, self).on_change_unit_price()
        except AttributeError:
            pass
#            result = {}
        self.rebate = self._compute_rebate(self.list_price, self.unit_price)
